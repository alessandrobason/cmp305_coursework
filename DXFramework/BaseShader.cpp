...
void BaseShader::loadVertexShader(const wchar_t *filename) {
	...
	loadVertexShader(filename, polygonLayout, sizeof(polygonLayout) / sizeof(polygonLayout[0]));
}


void BaseShader::loadTextureVertexShader(const wchar_t *filename)
{
	ID3DBlob *vertexShaderBuffer = loadShader(filename);
...
{
	ID3DBlob *pixelShaderBuffer = loadShader(filename);
...
{
	ID3D10Blob *hullShaderBuffer = loadShader(filename);
...
{
	ID3D10Blob *domainShaderBuffer = loadShader(filename);
...
{
	ID3D10Blob *geometryShaderBuffer = loadShader(filename);
...
{
	ID3D10Blob *computeShaderBuffer = loadShader(filename);
...
ID3DBlob *BaseShader::loadShader(const wchar_t *filename) {
	...
	if (idx != std::string::npos) {
		...
		// No extension found
		MessageBox(hwnd, L"Error finding shader file", L"ERROR", MB_OK);
		exit(0);
	}

	if (extension != L"cso") {
		MessageBox(hwnd, L"Incorrect shader file type", L"ERROR", MB_OK);
		exit(0);
	}

	// Reads compiled shader into buffer (bytecode).
	HRESULT result = D3DReadFileToBlob(filename, &buffer);
	if (result != S_OK) {
...
void BaseShader::loadVertexShader(const wchar_t *filename, D3D11_INPUT_ELEMENT_DESC layoutDesc[], unsigned int layoutLen) {
	ID3DBlob *vertexShaderBuffer = loadShader(filename);
	...
	// Create the vertex input layout.
	renderer->CreateInputLayout(layoutDesc, layoutLen, vertexShaderBuffer->GetBufferPointer(), vertexShaderBuffer->GetBufferSize(), &layout);
