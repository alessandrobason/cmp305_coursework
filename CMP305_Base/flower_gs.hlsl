...
Texture2D flowermap : register(t1);
...
void addGrass(InputType v1, InputType v2, float3 normal, inout TriangleStream<OutputType> triStream) {
    ...
    float grassHeight = 3.0 * 1.3;
    float grassLen = 2.3 * 1.3;
    ...
    float4 topRight = bottomRight + norm * grassHeight;
    ...
    tangent = float4(cross(horParallel.xyz, normal.xyz), 1.0);
...
void main(triangle InputType input[3], inout TriangleStream<OutputType> triStream) {
	...
	float4 flowers = flowermap.SampleLevel(sampler0, uv, 0);
