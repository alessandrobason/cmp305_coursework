...
void TerrainMesh::BuildHeightMap() {
	...
	for( int j = 0; j < ( resolution ); j++ ) {
		for( int i = 0; i < ( resolution ); i++ ) {
			height = ( sinf( (float)i * 0.1f * scale ) ) * 10.0f;
			height += ( cosf( (float)j * 0.033f * scale +1.0f) ) * 10.0f;
...
void TerrainMesh::Regenerate( ID3D11Device * device, ID3D11DeviceContext * deviceContext ) {
	...
	for( j = 0; j < resolution; j++ ) {
		for( i = 0; i < resolution; i++ ) {
			...
			float mag = sqrtf( ( smoothedNormal.x * smoothedNormal.x ) + ( smoothedNormal.y * smoothedNormal.y ) + ( smoothedNormal.z * smoothedNormal.z ) );
